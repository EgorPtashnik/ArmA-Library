...
ArmADocs.register("Mission", {
    ...
    "EP_fnc_showObjects": {
        description: "Toggles simulation, visibility, captive state and damage on one or more objects / units at once. " +
                     "Accepts a mission-layer name (string), a group, a single object, or an array of objects.",
        syntax: "[ref, show?] call EP_fnc_showObjects",
        params: [
            { name: "ref",  type: "String | Group | Object | Array", desc: "Mission layer name, group (its units), single object, or explicit array of objects." },
            { name: "show", type: "Boolean (optional)",             desc: "true \u2192 enable simulation, show, non-captive, damage allowed. false \u2192 the opposite. Default true." }
        ],
        returns: "Array \u2014 the resolved objects.",
        example: '"MyAmbushLayer" call EP_fnc_showObjects;                 // enable\n' +
                 '["MyAmbushLayer", false] call EP_fnc_showObjects;         // disable\n' +
                 '[group player, false] call EP_fnc_showObjects;'
    }
